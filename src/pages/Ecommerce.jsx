...
const Ecommerce = () => {
  ...
  const formatCurrencyData = (currencyData) => {
    return Object.entries(currencyData).map(([currency, value]) => ({
      title: currency,
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (data) {
        const processedData = formatCurrencyData(data);
        setEarningData(processedData);
        setVisibleCurrencies(processedData.slice(0, 3).map(item => item.title));
      }
    };
    fetchData();
  }, []);
    ...
    <div className="mt-24 mb-24 bg-white dark:text-gray-200 dark:bg-secondary-dark-bg">
      ...
      <div>
        ...
        <CurrencyDropDown
          ...
          setVisibleCurrencies={setVisibleCurrencies}
        />
        </div>
        <div>
          <CurrencyDisplay
